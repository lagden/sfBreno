...
class Pagination
{
    public static function show($page=1,$pages=1,$route=null,$lang='pt-BR')
    {
        $adjacents=3;
        $prev=$page-1;
        $next=$page+1;
        ...
        $anterior='<button type="button" data-pagina="'.url_for($route,array('pagina' => $prev)).'" class="prior paginacao paginacaoUI">'.$anteriorLang[$lang].'</button>';
        $anteriorD='<button type="button" class="priorD paginacaoDisabled paginacaoUI">'.$anteriorLang[$lang].'</button>';
        $proximo='<button type="button" data-pagina="'.url_for($route,array('pagina' => $next)).'" class="next paginacao paginacaoUI">'.$proximoLang[$lang].'</button>';
    ...
    protected static function countPage($counter,$page,$route){
        ...
        else return '<button type="button" data-pagina="'.url_for($route,array('pagina' => $counter)).'" class="paginacao paginacaoUI">'.$counter.'</button>';
    }

    protected static function lastPage($lpm,$lastpage,$route){
        return '
            <button type="button" class="paginacaoUI">...</button>
            <button type="button" data-pagina="'.url_for($route,array('pagina' => $lpm)).'" class="paginacao paginacaoUI">'.$lpm.'</button>
            <button type="button" data-pagina="'.url_for($route,array('pagina' => $lastpage)).'" class="paginacao paginacaoUI">'.$lastpage.'</button>
        ';
    }

    protected static function oneTwo($route){
        return '
            <button type="button" data-pagina="'.url_for($route,array('pagina' => 1)).'" class="paginacao paginacaoUI">1</button>
            <button type="button" data-pagina="'.url_for($route,array('pagina' => 2)).'" class="paginacao paginacaoUI">2</button>
            <button type="button" class="paginacaoUI">...</button>
        ';
    }
}